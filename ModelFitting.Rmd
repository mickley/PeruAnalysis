...
```{r unimodel_confints, eval=!usecluster}
...
```{r bimodel_confints, eval=!usecluster}
...
```{r exploring_sapling_model_results, eval =!usecluster}
...
anovaSaps2 <- 
    bootstrap.compare.lme(sapMod.uni, term="huntpres:HSD",
                          dists=list(1:5, 1:10, 1:rmax, 5:10), nboot=19, ncore=7)

anova.saps<- c(anovaSaps1, "huntpres:HSD" = list(anovaSaps2))
...
load("../results/v7.0/Peru_v7_1_wayAnovaSaps_rmax15_nsim999.RData")
load("../results/v7.0/Peru_v7_2_wayAnovaSaps_rmax15_nsim999.RData")

load("../results/v7.0/Peru_v7_1_wayAnovaSaps_rmax15_nsim999_NoAbiotic.RData")
load("../results/v7.0/Peru_v7_2_wayAnovaSaps_rmax15_nsim999_NoAbiotic.RData")

## univariate
anova.saps.uni.1way <- 
  lapply(anovaSaps1$uni$anovas, function(x) x$dtable)
...
saplingAnovaTable.uni
saplingAnovaTable.bi
```
...
hyperdat.uni.sel.c2 <- subset(hyperdat.uni.sel.c, Abiotic!=1)
...
resids.bi$N <- rep(hyperdat.bi.sel.c$N1, 15)
...
### 3.3 Interactive model confidence intervals
...
preddat.int <-  expand.grid(stage=c('S', 'J'), 
                            huntpres=quantile(sitedat$huntpres, c(0.25, 0.75)),
                            disp = levels(hyperdat.uni.sel.c$disp))

## RHS of model formula
allform <- update(formula(intMod.unidisp[[1]]), NULL~.)
## Make model matrix
modmat.int <-  model.matrix(allform, data=preddat.int)

## Bootstrapping
## Univariate
system.time(intMod.uni.boot <-  bootstrap.t.CI.lme(intMod.unidisp, lin.comb.Ct=modmat.int, nboot=nsim,
                                       alpha=0.05, ncore=7))
...
#### 3.3.1 Obtaining the results from the cluster
...
#### 3.3.2 Plotting the coefficients
...
### 3.4 Formal tests of hypotheses 2 and 3
...
load("../results/v7.0/Peru_v7_1_wayAnovaJuvs_rmax15_nsim999.RData")
load("../results/v7.0/Peru_v7_2_wayAnovaJuvs_rmax15_nsim999.RData")
load("../results/v7.0/Peru_v7_3_wayAnovaJuvs_rmax15_nsim999.RData")
...
35/224; 29/176
131/224; 110/176 ## proportion large vertebrate dispersed
