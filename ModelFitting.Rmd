...
`r subset(hyperdat.uni.sap, sp.id %in% c(764, 446, 562, 411), select=c('sp.id', 'Spp', 'N1', 'pname'))`
...
#### 2.3.4 Cluster results
```{r clusterresults, evaluate=TRUE}
## Alternatively, load results from cluster
load("../data/clusterResults/PeruDispersal_v7_rmax15.RData")
dim(results$bi$boot$modelpars)
sapMod.uni.boot <- results$uni$boot
sapMod.bi.boot <- results$bi$boot
```
...
```{r H1hypothesistests, evaluate=FALSE}
...
```{r load_anova}
load("../data/clusterResults/Peru_v7_1_wayAnovaSaps_rmax15.RData")
load("../data/clusterResults/Peru_v7_2_wayAnovaSaps_rmax15.RData")
...
Once again, we can run the bootstrapping on a cluster and use those results here.

```{r clusterresultsJuvs, evaluate=TRUE}
## Alternatively, load results from cluster
load("../data/clusterResults/PeruDispersal_v7_rmax15.RData")
...
int.bi.plotdat <- kfunclme2plot(intMod.bi.boot, preddat=results$preddat)
int.uni.plotdat <- kfunclme2plot(intMod.uni.boot, preddat=results$preddat)
