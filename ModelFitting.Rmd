...
if(windsep)
  load("../results/v7.1/PeruDispersal_v7_rmax15_NoAbiotic.RData") else
...
int.plotdat <- subset(int.plotdat, huntpres %in% c(-3, 0.1, 2.9) & HSD %in% c(0, 0.5, 1))
...
pl.int.uni <- 
  ggplot(int.plotdat, aes(x=distance, y=K.uni, ymin=lcl.uni, ymax=ucl.uni,
             colour=stage, fill=stage)) +
  geom_ribbon(alpha=0.5, colour=NA) +  geom_line() +
  facet_grid(HSD~  huntpres, as.table=FALSE) +
  geom_abline(intercept=0, slope=0, linetype='dashed', col='grey') +
...
z.bi <- gtable_add_grob(z.bi,
                        list(rectGrob(gp = gpar(fill=NA, col=NA, lwd=0)),
                             textGrob("Disperser sensitivity (HSD)", rot = -90,
                                      gp = gpar(fontsize=10))),
...
z.uni <- gtable_add_rows(z.uni, height=unit(z.uni$heights[[6]], 'cm'),  pos=5)
z.uni <- gtable_add_rows(z.uni, height=unit(z.uni$heights[[6]], 'cm'),  pos=5)
z.uni <- gtable_add_cols(z.uni, width=unit(z.uni$widths[[9]], 'cm'), pos=9)
z.uni <- gtable_add_cols(z.uni, width=unit(z.uni$widths[[9]], 'cm'), pos=9)
...
if(clusterNiter >= 999){
  ...
  pdf(file='../../huntingpaper/JEcol/figures/revision/Figure1_uni.pdf', width=6, height=4)
  ggdraw(z.uni)
  dev.off()
...
```{r indKfuncs_spsel}
hyperdat.uni.ind <- subset(hyperdat.uni.sel.c, N1 > 15)
...
# that if a site has a species there will be 2 rows (saps + juvs)
...
## Six most abundance species
sp2plot <- disptype$Spp[1:4]
...
sp.mods <- lapply(split(ind.funcs, f=ind.funcs$Spp), function(sp.dat){
  sp.mod <- lmHyperframe(sp.dat, 0:15, "stage*hunted", computeK=FALSE, minsamp=NA)
  preddat <- expand.grid(stage=levels(sp.dat$stage), hunted=levels(sp.dat$hunted))
  Xmat <- model.matrix(~stage*hunted, preddat)
  sp.boot  <- lm.t.boot(sp.mod,lincomb=Xmat, nsim=nsim, alpha=0.05, simple=FALSE)
  sp.dat <- with(sp.boot, {
    data.frame(
...
sp.mods$hunted <- factor(sp.mods$hunted, levels=c('intact', 'hunted'),
                           labels=c('Low', 'High'))
sp.mods$stage <- factor(sp.mods$stage, levels=c('S', 'J'),
...
sp.plot  <- ggplot(data=ind.funcs, aes(x=r, y=K)) +
  geom_ribbon(data=sp.mods, aes(ymin=lcl, ymax=ucl, colour=NULL, fill=hunted, group=hunted), alpha=0.5) + 
...
z.sp <- gtable_add_cols(z.sp, width=z.sp$widths[[11]], pos=12)
...
z.sp <- gtable_add_grob(z.sp,
                     list(rectGrob(gp = gpar(fill=NA, col=NA, lwd=0)),
                          textGrob("Relative conspecific clustering", rot = 90,
                                   gp = gpar(fontsize=12))),
                     t=7, l=2, b=11, r=2, name = paste(runif(2)))
...
z.sp <- gtable_add_grob(z.sp,
                     list(rectGrob(gp = gpar(fill='grey', col='grey', lwd=0.2)),
                          textGrob("Stage", rot = -90,
                                   gp = gpar(fontsize=10))),
                     t=8, l=14, b=10, r=14, name = paste(runif(2)))

ggdraw(z.sp)

pdf('../../huntingpaper/figures/Figure1_bysp.pdf', width=8, height=4)
ggdraw(z.sp)
dev.off()
png('../../huntingpaper/figures/Figure1_bysp.png', width=8, height=4, units='in', res=600)
ggdraw(z.sp)
dev.off()
```
...
```{r SAuppFigspFuncs, warning=FALSE}
...
plotgrps <- list(hsdbysp$splab[1:9], hsdbysp$splab[10:18], hsdbysp$splab[19:26])
...
pdf('../../huntingpaper/figures/FigS1_byspecies_3pg.pdf', paper='A4', width=7, height=10)
